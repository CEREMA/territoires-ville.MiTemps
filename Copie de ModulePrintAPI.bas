...
'Variables globales privées pour stocker les options d'impression
Private uneValOrientation As Integer, uneValCopies As Integer
Private uneValDuplex As Integer, uneValPaperSize As Integer
Private uneValColorMode As Integer, uneValZoom As Integer
Private uneValPrintQuality As Integer, unNomDevPrinter As String * CCHDEVICENAME
...
Public Sub ShowPrinter(frmOwner As Form, Optional PrintFlags As Integer)
    ...
    'Toutes ces valeurs sont nulles au premier appel
    If unNbAppel = 0 Then
        'Premier appel de cette fonction ShowPrinter
        'Initialisation des valeurs avec le printer par défaut
        unNbAppel = 1
        With Printer
            uneValOrientation = .Orientation
            uneValCopies = .Copies
            uneValDuplex = .Duplex
            uneValPaperSize = .PaperSize
            uneValColorMode = .ColorMode
            uneValZoom = .Zoom
            uneValPrintQuality = .PrintQuality
            unNomDevPrinter = .DeviceName
        End With
    End If
    ...
    DevMode.dmDeviceName = unNomDevPrinter 'Printer.DeviceName
    DevMode.dmSize = Len(DevMode)
    'On initialise avec les dernières valeurs saisies lors de l'appel précédent
    ...
    'end with
    DevMode.dmOrientation = uneValOrientation
    DevMode.dmCopies = uneValCopies
    DevMode.dmDuplex = uneValDuplex
    DevMode.dmPaperSize = uneValPaperSize
    DevMode.dmColor = uneValColorMode
    DevMode.dmScale = uneValZoom
    DevMode.dmPrintQuality = uneValPrintQuality
    On Error GoTo 0
    ...
    If PrintDialog(PrintDlg) Then
        ...
        lpDevName = GlobalLock(PrintDlg.hDevNames)
            CopyMemory DevName, ByVal lpDevName, Len(DevName) '45
        bReturn = GlobalUnlock(lpDevName)
        ...
        DoEvents
        With Printer
            'MsgBox "Avant .Copies = DevMode.dmCopies"
            .Copies = DevMode.dmCopies
            'MsgBox "Avant .Duplex = DevMode.dmDuplex"
            .Duplex = DevMode.dmDuplex
            'MsgBox "Avant .Orientation = DevMode.dmOrientation"
            .Orientation = DevMode.dmOrientation
            'MsgBox "Avant .PaperSize = DevMode.dmPaperSize"
            .PaperSize = DevMode.dmPaperSize
            'MsgBox "Avant .ColorMode = DevMode.dmColor"
            .ColorMode = DevMode.dmColor
            'MsgBox "Avant .Zoom = DevMode.dmScale"
            .Zoom = DevMode.dmScale
            'MsgBox "Avant .PrintQuality = DevMode.dmPrintQuality"
            .PrintQuality = DevMode.dmPrintQuality
        End With
        
        'Stockage des valeurs pour les prochains appels de ShowPrinter
        'dans la même session de l'application
        With Printer
            uneValOrientation = .Orientation
            uneValCopies = .Copies
            uneValDuplex = .Duplex
